  ...
  return (
    <div className="flex justify-center items-center min-h-[calc(100vh-80px)] p-6">
      <div className="w-full max-w-[450px] p-10 bg-white rounded-xl shadow-lg border border-border">
        <h1 className="text-3xl font-bold mb-2 text-center text-text-primary">Join Medium Clone</h1>
        <p className="text-center text-text-secondary mb-8 text-[0.95rem]">Create an account to start writing</p>

        {error && <div className="bg-red-50 text-error p-4 rounded-lg mb-6 border-l-4 border-error text-sm">{error}</div>}

        <form onSubmit={handleSubmit} className="mb-6">
          <div className="mb-6">
            <label htmlFor="username" className="block mb-2 font-semibold text-sm text-text-primary">Username</label>
            <input
              type="text"
              id="username"
              value={username}
