...
const Home = () => {
  ...
  if (loading) {
    return <div className="max-w-[728px] mx-auto py-8 px-6 text-center text-text-secondary">Loading articles...</div>;
  }
  ...
  return (
    <div className="max-w-[728px] mx-auto py-8 px-6">
      <div className="mb-12 pb-8 border-b border-border">
        <h1 className="text-4xl font-bold mb-2 text-text-primary">Latest Stories</h1>
        <p className="text-lg text-text-secondary">Discover stories, thinking, and expertise from writers on any topic.</p>
      </div>
            ...
            <article key={article._id} className="pb-10 border-b border-border transition-all hover:translate-x-1 duration-300 last:border-b-0 group">
              <Link to={`/articles/${article._id}`}>
                ...
                <div className="flex items-center gap-4 text-sm">
                  <Link to={`/profile/${article.author.username}`} className="text-text-primary font-semibold hover:text-primary transition-colors">
